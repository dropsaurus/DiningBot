...
// GENERATE A PERSISTENT SESSION ID (once when page loads)
var lexSessionId =
  "chatbot-session-" +
  Date.now() +
  "-" +
  Math.random().toString(36).substring(7);
console.log("Session ID for this conversation:", lexSessionId);
...
$(document).ready(function () {
  ...
  function callChatbotApi(message) {
    ...
    console.log("Sending to API with session:", lexSessionId);
    ...
    return apigClient.chatbotPost(
      {},
      {
        ...
        sessionId: lexSessionId, // SEND SESSION ID TO BACKEND
      },
      {}
    );
  }
      ...
      .then((response) => {
        ...
        if (data.messages && data.messages.length > 0) {
          ...
          for (var message of messages) {
            ...
            if (message.type === "unstructured") {
              console.log("Bot says:", message.unstructured.text);
              insertResponseMessage(message.unstructured.text);
            } else if (
              message.type === "structured" &&
              message.structured.type === "product"
              ...
              console.log("not implemented message type:", message.type);
            }
          }
        } else {
  ...
  function insertResponseMessage(content) {
    console.log("Inserting bot message:", content);
    $(
      '<div class="message loading new"><figure class="avatar"><img src="https://media.tenor.com/images/4c347ea7198af12fd0a66790515f958f/tenor.gif" /></figure><span></span></div>'
    ).appendTo($(".mCSB_container"));
    updateScrollbar();
